...
Sivaramachandran Ganesan \hfill Jan. 26, 2013
  ...
  \item
    \begin{enumerate}
      \item First, we create the augmented CFG and the postdominator
        ...
        \newpage
      \item Below, regions (indicated as trapezoids) are added to the
        control dependence graph. This was performed in one pass
        through the control dependence graph: for each node $n$ in the
        graph, one region was created to group the nodes dependent on
        $n$ evaluating to |true|, if any, and another region to group
        the nodes dependent on $n$ evaluating to |false|, if any. Each
        region was made the parent of the nodes in its corresponding
        group, and the region in turn was made a child of $n$.
