...
\begin{enumerate}
  ...
  \item We start by augmenting the CFG with an edge from entry to
    start, and then reverse it (below, left). From that we create the
    dominator tree (below, right).
    ...
    \begin{minipage}[b]{.5\linewidth}
      \begin{center}
        \begin{tikzpicture}[on grid, node distance=1 and 1]
          ...
          \draw (10) -- (9);
          \draw (9) -- node[swap, near start] {T} (8);
          \draw (13) -- (12);
          \draw (12) -- node[near start] {F} (8);
          \path (8) edge[out=180,in=-60] node[near end, swap] {F} (4);
    ...
    \begin{minipage}[b]{.45\linewidth}
      \begin{center}
        \begin{tikzpicture}[on grid,node distance=1 and 1]
          ...
          \node[block, below left=of 4] (3) {3};
          \node[block, below left=1.2 and .6 of 3] (2) {2};
          \node[block, below=of 2] (1) {1};
    ...
    a child of $n$ in the RCFG but not in the dominator tree. This set
    of nodes is the dominance frontier of $n$. These are tabulated
    below.\todo{mention condition if including}
      ...
      \begin{minipage}[b]{.8\linewidth}
        \begin{center}
          \begin{tikzpicture}[node distance=1.4 and .8]
            ...
            edge node[near end] {T} (16)
            edge node[near end,right=.1] {T} (17)
            edge[loop left] node[below=.1] {T\todo{include?}} (15);
